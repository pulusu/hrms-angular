 ...
 //title = 'materialApp';   
   ...
   ngOnInit() {
    ...
    this.currentTrackUser = JSON.parse(localStorage.getItem('hrms-kairos-currentTrackUser'));
         ...
         'employee_pancard': [null, Validators.required],
         'employee_aadhar_card': [null, Validators.required],
         'employee_uan_number': [null, Validators.required],
  ...
  this.http.post(`${environment.api_url}`+`${environment.employee_add}`, obj3).subscribe((form:any)=>{
    ...
    //  mailData['subject']='Welcome To Kairos Technologies'; 
     // mailData['from_mail']=this.currentTrackUser.employee_office_email; 
      //mailData['to_mail']=this.employeeForm.value.employee_personal_email;
      //mailData['text'] ='Hi '+this.employeeForm.value.employee_name+', <br> <p>Your Login Deatils For Our office Portal</p><p><b>User Name </b>:'+this.employeeForm.value.employee_personal_email+'</p><p><b>Password </b>:'+this.employeeForm.value.employee_id+'</p>';
...
onChanges() {
 ...
 this.secondFormGroup.get('employee_father_dob').valueChanges
  .subscribe(selectedValue => { 
         //alert(selectedValue);
		 //alert(new Date(this.birthdate));
		 this.birthdate = selectedValue; 
        ...
        //this.data.age = 
		//alert(Math.floor((timeDiff / (1000 * 3600 * 24))/365));
